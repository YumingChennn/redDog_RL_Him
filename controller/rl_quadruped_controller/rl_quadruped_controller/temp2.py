...
from sensor_msgs.msg import Imu  # 改成 Imu 訊息
...
class AngularVelocityProcessor(Node):
    def __init__(self, cutoff=1.5, fs=100, order=8):
        ...
        self.imu_subscription = self.create_subscription(
            Imu,  # 改成 Imu 訊息
            '/low_level_info/imu/data_raw',  # 訂閱正確的 IMU 話題
            self.imu_callback,
            10
        )
        self.get_logger().info("Subscribed to /low_level_info/imu/data_raw")
    ...
    def imu_callback(self, msg):
        # 記錄原始數據
        self.raw_imu_log['x'].append(msg.angular_velocity.x)
        self.raw_imu_log['y'].append(msg.angular_velocity.y)
        self.raw_imu_log['z'].append(msg.angular_velocity.z)

        self.imu_buffer['x'].append(msg.angular_velocity.x)
        self.imu_buffer['y'].append(msg.angular_velocity.y)
        self.imu_buffer['z'].append(msg.angular_velocity.z)
